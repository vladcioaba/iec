...
public class Evaluate extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException,
		...
		// We shall parse the array of answers and check how many are wrong / correct
		...
		int[][] currentAnswers = new int[allTests.size()][2];	// cAns[][0] -> his answer 
																// cAns[][1] -> severity of q
		...
		for (Test t : allTests) {
			...
			progressValue += this.calculateProgressValue(
					currentAnswers[iterator], t.getTestCorrectAnswerValue(), 1
			);
			...
			DatabaseManager.setTestComplete(
					currentUserId, 
					t.getTestId(), 
					currentAnswers[iterator][0], 
					(Date) session.getAttribute("start"), 
					(Date) session.getAttribute("stop")
			);
		}
		...
		if (DatabaseManager.isFirstTime(currentUserId)) {
			...
			// now we balance the current answer with the positive/negative answers from the past
			...
			for (UserTest historicTestAnswer : historicTestAnswers) {
				...
				progressValue += this.calculateProgressValue(
						thisTestValues, foundTest.getTestCorrectAnswerValue(), 3
				);
			}
			
			int currentLesson = Integer.parseInt((String) session.getAttribute("noLesson"));
	...
	private int calculateProgressValue(int[] answerVectAndSeverity, int correctAnswer, int byHowMuch) 
		...
		if (answerVectAndSeverity[0] == correctAnswer) {
			...
			switch(answerVectAndSeverity[1]) {
				...
				default:
					throw (new ServletException("Severity not found for " 
							+ "answerVectAndSeverity=" + answerVectAndSeverity
